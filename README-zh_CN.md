...
- `.choices({val1, val2, ...})`: 将参数限制为指定的选项之一。对于字符串类型的选项，传入字符串的初始化列表。
  ```cpp
  std::string mode;
  parser.add_option("mode", "操作模式", mode)
        .choices({"fast", "slow", "balanced"});
  ```
- `.choices({{val1, desc1}, {val2, desc2}, ...})`: 将参数限制为指定的选项之一，并在帮助信息中显示每个选项的描述文本。对于字符串类型的选项，传入 `std::map<std::string, std::string>`。对于非字符串类型（数字、自定义类型），这是唯一支持的形式，且类型必须可比较（`operator<`）。
  ```cpp
  std::string optimization;
  parser.add_option("opt", "优化级别", optimization)
        .choices({{"0", "无优化"}, {"1", "基本优化"}});
  int level;
  parser.add_option("level", "调试级别", level)
        .choices({{0, "无调试"}, {1, "基本调试"}, {2, "详细调试"}});
  ```
...
- `.validator(validation_function)`: 提供自定义验证逻辑。验证函数接受一个值并返回一个 `std::pair<bool, std::string>`。如果 `bool` 为 `false`，解析将失败，并将 `std::string` 用作错误消息。
  - `validator(std::function<std::pair<bool, std::string>(const std::string&)>)`: 在解析前对原始字符串值进行操作。
  - `validator(std::function<std::pair<bool, std::string>(const ValueType&)>)`: 对解析后的值进行操作（`ValueType` 是解包后的类型，例如 `std::optional<int>` 的 `int`）。
...
- `.hidden()`: 在帮助信息中隐藏选项或位置参数。
  ...
  parser.add_option("internal-setting", "仅供内部使用", settings.internal)
        .hidden();
  ```
- `.value_placeholder(placeholder_string)`: 自定义帮助信息中参数值的占位符字符串（例如 `<FILE>`、`<NUMBER>`）。
  ```cpp
...
// 解析器将按顺序期望它们。如果需要，可使用 .validator() 进行进一步验证。
...
**修饰符：**
位置参数可以使用前面描述的\u201c选项（和位置参数）的通用修饰符\u201d（例如 `.default_value()`、`.choices()`、`.validator()`、`.env()`、`.hidden()`、`.value_placeholder()`、`.callback()`）。
...
- **语法**: `argparse::set_option_name_width(width_in_characters);`

**示例：**

```cpp
argparse::ArgParser parser("my_app", "我的应用程序");
argparse::set_option_name_width(40); // 为选项名称列分配40个字符
// ... 添加参数 ...
```

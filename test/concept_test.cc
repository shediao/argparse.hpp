...
// 测试 can_parse_from_string_without_split concept
TEST(ConceptTest, CanParseFromStringWithoutSplit) {
    ASSERT_TRUE(!argparse::can_parse_from_string_without_split<char>);
    ASSERT_TRUE(!argparse::can_parse_from_string_without_split<wchar_t>);
    ASSERT_TRUE(!argparse::can_parse_from_string_without_split<char16_t>);
    ASSERT_TRUE(!argparse::can_parse_from_string_without_split<char32_t>);
    ASSERT_TRUE(argparse::can_parse_from_string_without_split<bool>);
    ASSERT_TRUE(argparse::can_parse_from_string_without_split<int>);
    ASSERT_TRUE(argparse::can_parse_from_string_without_split<double>);
    ASSERT_TRUE(argparse::can_parse_from_string_without_split<std::string>);
    ASSERT_TRUE(
        !argparse::can_parse_from_string_without_split<std::vector<int>>);
}

// 测试 can_parse_from_string_split_once concept
TEST(ConceptTest, CanParseFromStringSplitOnce) {
    ASSERT_TRUE(
        (argparse::can_parse_from_string_split_once<std::pair<int, int>>));
    ASSERT_TRUE((argparse::can_parse_from_string_split_once<
                 std::tuple<int, std::string>>));
    ASSERT_TRUE(
        (argparse::can_parse_from_string_split_once<std::array<int, 8>>));
    ASSERT_TRUE(!argparse::can_parse_from_string_split_once<int>);
    ASSERT_TRUE(!argparse::can_parse_from_string_split_once<std::vector<int>>);
}

...
TEST(FromStringTest, IntTest) {
    ...
    EXPECT_THROW(from_string<int>(std::to_string(std::numeric_limits<long long>::max())), std::out_of_range);
}
...
TEST(FromStringTest, LongTest) {
    ...
    if constexpr (sizeof(long) != sizeof(long long)) {
        EXPECT_THROW(from_string<long>(std::to_string(std::numeric_limits<long long>::max())), std::out_of_range);
    }
}
...
TEST(FromStringTest, DoubleTest) {
    ...
    EXPECT_THROW(from_string<double>("1" + std::to_string(std::numeric_limits<double>::max())), std::out_of_range);
}
...
TEST(FromStringTest, LongLongTest) {
    ...
    EXPECT_THROW(from_string<long long>(std::to_string(std::numeric_limits<long double>::max())), std::out_of_range);
}

TEST(FromStringTest, Split) {
    EXPECT_EQ(detail::split("k=v",'=',2), (std::vector<std::string>{"k", "v"}));
    EXPECT_EQ(detail::split("1,2,3",',',3), (std::vector<std::string>{"1", "2", "3"}));
    EXPECT_EQ(detail::split("1,2,3",',',2), (std::vector<std::string>{"1", "2,3"}));
    EXPECT_EQ(detail::split("1,2,3",',',4), (std::vector<std::string>{"1", "2", "3"}));
}

TEST(FromStringTest, Sizeable) {
    EXPECT_EQ((from_string<std::pair<int, int>>("1,2", ',')),(std::pair<int,int>{1, 2}));
    EXPECT_EQ((from_string<std::tuple<int, int, int> >("1,2,3", ',')),(std::tuple{1,2,3}));

    EXPECT_EQ((from_string<std::pair<std::string, std::string>>("k=v", '=')),(std::pair<std::string,std::string>{"k", "v"}));
    EXPECT_EQ((from_string<std::pair<std::string, std::string>>("k=", '=')),(std::pair<std::string,std::string>{"k", ""}));
    EXPECT_EQ((from_string<std::pair<std::string, std::string>>("k=v=vv", '=')),(std::pair<std::string,std::string>{"k", "v=vv"}));

    EXPECT_THROW((from_string<std::pair<std::string, std::string>>("k", '=')),std::invalid_argument);
}

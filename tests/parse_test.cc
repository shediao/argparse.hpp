...
TEST_F(ArgParserTest, MixedFlagTest) {
  auto args =
      make_args("prog", {"-v", "--verbose", "-q", "--quiet", "--inc", "--dec"});
...
TEST_F(ArgParserTest, IntFlagTest2) {
  auto args =
      make_args("prog", "-v", "-v", "--verbose", "--inc", "--inc", "--no-inc");
  int count1 = 0, count2 = 0, count3 = 0;
...
TEST_F(ArgParserTest, DoubleDashPositionalTest) {
  auto args = make_args("prog", "-v", "--name=test", "--", "-v", "--name=value",
                        "pos1");
  bool verbose = false;
  std::string name;
  std::vector<std::string> positionals;
...
TEST_F(ArgParserTest, MapAndVectorPairDefaultValueTest) {
  auto args = make_args("prog", "--config=host=localhost", "--config=port=8080",
                        "--pairs=key1:value1", "--pairs=key2:value2",
                        "--attributes", "name=alice", "--attributes", "age=20");
...
TEST_F(ArgParserTest, InvalidOptionNameStartWithDashTest) {
  ...
  EXPECT_THROW(parser.add_positional("-input", "Invalid positional name", pos),
               std::invalid_argument);
}
...
TEST_F(ArgParserTest, SubCmdTest1) {
  auto args =
      make_args("git", {"-c", "user.name=Haha", "-c", "user.email=haha@xx.com",

...
- `.choices({val1, val2, ...})`: Restricts the argument to one of the specified choices. For string-valued options, pass an initializer list of strings.
  ```cpp
  std::string mode;
  parser.add_option("mode", "Operation mode", mode)
        .choices({"fast", "slow", "balanced"});
  ```
- `.choices({{val1, desc1}, {val2, desc2}, ...})`: Restricts the argument to one of the specified choices, with descriptive help text for each choice shown in the help message. For string-valued options, pass a `std::map<std::string, std::string>`. For non-string types (numeric, custom types), this is the only supported form, and the type must be comparable (`operator<`).
  ```cpp
  std::string optimization;
  parser.add_option("opt", "Optimization level", optimization)
        .choices({{"0", "No optimization"}, {"1", "Basic optimization"}});
  int level;
  parser.add_option("level", "Debug level", level)
        .choices({{0, "No debug"}, {1, "Basic debug"}, {2, "Verbose debug"}});
  ```
...
- **Ordering Multiple Positionals:**
  - Positional arguments bound to non-container types or compile-time sized containers must be defined _before_ any variadic positional argument.
...
- **Option values** with defined `.choices()` will suggest those choices.
